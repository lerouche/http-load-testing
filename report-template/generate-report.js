...
class ReportGenerator {
    ...
    getSystemInfo() {
        ...
        let lines = FileSystem.readFileSync(Path.join(this.rootPath, 'system.info'), 'utf8').split(/[\r\n]+/).map(line => line.trim()).filter(line => !!line);
        let data = {};
        lines.forEach(line => {
    ...
    getTestsList() {
        ...
        let dirlist = FileSystem.readdirSync(Path.join(this.rootPath, 'results')).filter(file => FileSystem.lstatSync(Path.join(this.rootPath, 'results', file)).isDirectory());
    ...
    getSystemLoadData(test, subject) {
        ...
        let lines = FileSystem.readFileSync(Path.join(folder, 'system-load.csv'), 'utf8').split(/[\r\n]+/).map(line => line.trim()).filter(line => !!line);
